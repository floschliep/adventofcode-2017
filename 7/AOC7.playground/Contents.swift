...
//    var heaviestTree: Tree<T> {
//        let sortedChildren = self.children.sorted(by: { $0.weight > $1.weight })
//        guard sortedChildren.count > 0 else { return self }
//        guard sortedChildren.count > 1 else { return sortedChildren[0] }
//        
//        return sortedChildren.map({ $0.heaviestTree }).sorted(by: { $0.weight > $1.weight })[0]
//    }
...
let newHeaviestWeight = heaviest.valueWeight - (heaviest.weight - other.weight) // 299
print(newHeaviestWeight)
